 ...
 * FIXED - More conservative and accurate California Method implementation 
...
export const lookupCulvertSizeFromTable = (width, depth) => {
  // FIXED: More conservative California Method lookup table
  ...
  if (width <= 2.0) {
    ...
    if (depth <= 0.2) return 900;   // FIXED: 2m × 0.2m → 900mm (was giving 2400mm!)
...
export const calculateCulvert = (params) => {
  ...
  // Step 4: Apply debris transport assessment
  ...
  // Run hydraulic calculations when needed
  ...
  if (needsHydraulicCalc) {
    ...
    for (const pipeSize of STANDARD_PIPE_SIZES) {
      ...
      // Simplified headwater calculation
      const velocity = bankfullFlow / pipeArea;
      const velocityHead = Math.pow(velocity, 2) / (2 * 9.81);
      const headwaterDepth = pipeDiameter * 0.5 + velocityHead + pipeDiameter * 0.1;
      ...
      // If we've reached the largest pipe size, use it
